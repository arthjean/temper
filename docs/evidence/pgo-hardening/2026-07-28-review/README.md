...
Each case retains the exact fixture source, workload and raw `run.json`. `summary.json` records the host/toolchain identity, baseline Git commit, the dirty production-input checksum, evidence-harness checksum, exact tested-binary checksum and independent artifact/profile checksum rechecks performed before temporary build directories were released.
