// Minimal SHA-256 shared by the EP-001 experiment programs so the harness needs
// no dependency and cannot change Temper's Cargo.lock. Included textually with
// `include!` because each harness program is compiled directly by rustc.
